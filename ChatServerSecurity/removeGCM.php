...
$response=array();
if(isset($_POST["id"]) && isset($_POST["root"])&&$_POST["root"]=="caputotavellamantovani99" &&isset($_POST["password"])){
			...
			if($result1){
				...
				if($result2){
					...
					if($update){
						...
						$prova=mysqli_query($connection,"UPDATE Chiavi SET gcmRegistration=null WHERE id_key='{$update['id_key']}'");
						$ora=date('Y-m-d G:i:s');
						$my=mysqli_query($connection,"UPDATE User SET created_at='{$ora}' WHERE id_user='{$id}'");
						if($prova){
